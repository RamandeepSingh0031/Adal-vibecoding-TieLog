      ...
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        <div className="relative">
          <input
            ...
            className="w-full px-4 py-3 pl-12 bg-[#2D2D2D]/50 border border-[#4A4A4A] rounded-xl text-[#F4F4F5] placeholder-[#71717A] focus:outline-none focus:border-[#14B8A6] backdrop-blur-sm transition-colors"
            autoFocus
          />
          ...
          <ul className="mt-6 space-y-2">
            {results.map((result) => (
              <li key={`${result.type}-${result.id}`}>
                <Link href={result.link} className="block p-4 bg-[#2D2D2D] border border-[#4A4A4A] rounded-xl hover:border-[#14B8A6]/30 transition-all hover:translate-x-1">
          ...
          <div className="mt-8">
            ...
            <div className="space-y-3">
              {clusters.map((cluster) => (
                <Link key={cluster.id} href={`/cluster/${cluster.id}`} className="block p-4 bg-[#2D2D2D] border border-[#4A4A4A] rounded-xl hover:border-[#14B8A6]/30 transition-all hover:translate-x-1">
        ...
        {showCreateForm && (
          <div className="mt-8 p-6 bg-[#2D2D2D] border border-[#4A4A4A] rounded-xl shadow-xl">
            ...
            <form onSubmit={handleCreateCluster} className="space-y-4">
              <input type="text" value={newClusterName} onChange={(e) => setNewClusterName(e.target.value)} placeholder="Cluster name" className="w-full px-4 py-3 bg-[#3E3E3E] border border-[#4A4A4A] rounded-xl text-[#F4F4F5] placeholder-[#71717A] focus:outline-none focus:border-[#14B8A6] transition-colors" autoFocus />
              <input type="text" value={newClusterDesc} onChange={(e) => setNewClusterDesc(e.target.value)} placeholder="Description (optional)" className="w-full px-4 py-3 bg-[#3E3E3E] border border-[#4A4A4A] rounded-xl text-[#F4F4F5] placeholder-[#71717A] focus:outline-none focus:border-[#14B8A6] transition-colors" />
